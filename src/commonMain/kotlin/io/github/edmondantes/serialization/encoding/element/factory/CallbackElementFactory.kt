...
public open class CallbackElementFactory(
    ...
    protected val callback: (EncodingElement<*>) -> Unit,
) : DescriptorElementFactory(descriptor, type, childDescriptor, parentDescriptor, indexInParent) {
    override fun <T : EncodingElement<*>> prepare(block: (SerialDescriptor?, SerialDescriptor?, SerialDescriptor?, Int?) -> T): T =
        super.prepare(block).also(callback)
}
